...
# two-phase classifier
# two-phase classifier
# This classifier treat with 5 classes excluding Others
# Non-profit organization, for-profit organization and Medias are treated as one classes in first phase
...
class TwoPhaseTwitterClassifier4:
    ...
    def train(self,class1,class2):
        ...
        for i in range(len(xy)):
            if xy[i][11]==2 or xy[i][11]==1:
                xy2[i][11]=0
    
        numberoftrain=len(xy)
        trainx,trainy=xy[:,:11],xy[:,11]
        trainx2,trainy2=xy2[:,:11],xy2[:,11]
    ...
    def test(self,xy):
        ...
        notest=len(predy1)
        preprototal=np.zeros((notest,5))
    
        for i in range(len(predy1)):
            preprototal[i][3]=predpro[i][1]
            preprototal[i][4]=predpro[i][2]
            if predy1[i]==0:
